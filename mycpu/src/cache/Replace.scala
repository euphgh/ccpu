...
class RandomReplacement(n_ways: Int) extends ReplacementPolicy {
  ...
  def cutRand(mod: Int, random: UInt): UInt = {
    if (isPow2(mod)) random(log2Ceil(mod) - 1, 0)
    else PriorityEncoder(partition(cutRand(1 << log2Up(mod * 8), random), mod))
  }

  def way  = cutRand(n_ways, lfsr)
...
class PseudoLRU(n_ways: Int) extends ReplacementPolicy {
  ...
  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    val touch_way_sized =
      if (touch_way.getWidth < log2Ceil(n_ways)) touch_way.pad(log2Ceil(n_ways))
